                ...
                for (int k = 1; k <= K; ++k) {
                    {
                        ...
                        if (as_bool(logical_eq(get_base1(prior_df,k,"prior_df",1),1))) {
                            ...
                            stan::math::assign(get_base1_lhs(beta,k,"beta",1), tan((stan::math::pi() * (P - 0.5))));
...
inv_gaussian(const Eigen::Matrix<T0__, Eigen::Dynamic,1>& y,
    ...
    int current_statement_begin__ = -1;
    try {
        return stan::math::promote_scalar<fun_return_scalar_t__>(((((0.5 * rows(y)) * log((lambda / (2 * stan::math::pi())))) - (1.5 * sum_log_y)) - ((0.5 * lambda) * dot_self(elt_divide(subtract(y,mu),elt_multiply(mu,sqrt_y))))));
    } catch (const std::exception& e) {
        stan::lang::rethrow_located(e,current_statement_begin__);
        // Next line prevents compiler griping about no return
        throw std::runtime_error("*** IF YOU SEE THIS, PLEASE REPORT A BUG ***");
    ...
    try {
        {
            ...
            stan::math::assign(ll, subtract(add(ll,(0.5 * log((lambda / (2 * stan::math::pi()))))),multiply(1.5,log_y)));
            return stan::math::promote_scalar<fun_return_scalar_t__>(ll);
        }
    } catch (const std::exception& e) {
        stan::lang::rethrow_located(e,current_statement_begin__);
...
public:
    ...
    void ctor_body(stan::io::var_context& context__,
        ...
        try {
            ...
            if (as_bool(logical_eq(family,1))) {
                stan::math::assign(sum_log_y, stan::math::not_a_number());
    ...
    void transform_inits(const stan::io::var_context& context__,
        ...
        for (int i0__ = 0U; i0__ < has_intercept; ++i0__)
            try {
            writer__.scalar_lb_unconstrain(((primitive_value(logical_eq(family,1)) || primitive_value(logical_eq(link,2))) ? stan::math::negative_infinity() : 0.0 ),gamma[i0__]);
        } catch (const std::exception& e) { 
            throw std::runtime_error(std::string("Error transforming variable gamma: ") + e.what());
        }
    ...
    T__ log_prob(vector<T__>& params_r__,
        ...
        for (size_t k_0__ = 0; k_0__ < dim_gamma_0__; ++k_0__) {
            if (jacobian__)
                gamma.push_back(in__.scalar_lb_constrain(((primitive_value(logical_eq(family,1)) || primitive_value(logical_eq(link,2))) ? stan::math::negative_infinity() : 0.0 ),lp__));
            else
                gamma.push_back(in__.scalar_lb_constrain(((primitive_value(logical_eq(family,1)) || primitive_value(logical_eq(link,2))) ? stan::math::negative_infinity() : 0.0 )));
        }
